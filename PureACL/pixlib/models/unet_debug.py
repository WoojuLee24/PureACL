...
from PureACL.pixlib.geometry.interpolation import interpolate_tensor
from PureACL.visualization.viz_2d import imsave
...
class UNet(BaseModel):
    ...
    def build_encoder(self, conf):
        ...
        encoder.load_state_dict(torch.load('/ws/external/checkpoints/pretrained/vgg16-397923af.pth'))
        ...
        if data['type'] == 'grd':
            ...
            if 1:
                ...
                r2q_img, r2q_mask, _ = interpolate_tensor(data['ref']['image'], p2d_ref, 'linear',
                                                          pad=4, return_gradients=False, out_shape='bcn')
                b, c, h, w = data['image'].shape
                r2q_img, r2q_mask = r2q_img.reshape(-1, c, h, w), r2q_mask.reshape(-1, 1, h, w)
                # debugging
                imsave(r2q_img[0], '/ws/external/visualizations/dense2', '1r2q')
                imsave(data['image'][0], '/ws/external/visualizations/dense2', '1grd')
                imsave(data['ref']['image'][0], '/ws/external/visualizations/dense2', '1sat')
            ...
            if 1:
                ...
                q2r_img, q2r_mask, _ = interpolate_tensor(data['query']['image'], p2d_q,
                                                          'linear', pad=4, return_gradients=False,
                                                          out_shape='bcn')
                b, c, h, w = data['image'].shape
                q2r_img, q2r_mask = q2r_img.reshape(-1, c, h, w), q2r_mask.reshape(-1, 1, h, w)
                imsave(q2r_img[0], '/ws/external/visualizations/dense2', '0q2r')
                imsave(data['image'][0], '/ws/external/visualizations/dense2', '0sat')
                imsave(data['query']['image'][0], '/ws/external/visualizations/dense2', '0grd')
