...
log_id_val = "2017-10-26-V2-Log4"
log_id_test = "2017-10-26-V2-Log5"
map_points_dir = 'pcd'
...
class _Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        camera = Camera.from_colmap(dict(
            model='SIMPLE_PINHOLE', params=(
            1 / meter_per_pixel, satellite_ori_size / 2.0, satellite_ori_size / 2.0, 0, 0, 0, 0,
            np.infty),  # np.infty for parallel projection
            width=int(satellite_ori_size), height=int(satellite_ori_size)))
        sat_image = {
            'image': sat_map.float(),
            'camera': camera.float(),
        ...
        if self.conf.pose_from == 'aa':
            ...
            R = np.array(
                [[np.cos(heading), -np.sin(heading), 0], [np.sin(heading), np.cos(heading), 0], [0, 0, 1.]], dtype=np.float32)
            t = np.zeros(3, dtype=np.float32)
            body2ned = Pose.from_Rt(R, t)

        body2sat = ned2sat@body2ned
        body2sat.t[-1] = 0  # no shift on height

        # offset is added
        translation_offset = np.array([[1., 0, 0, dx], [0, 1, 0, -dy], [0, 0, 1, 0],
                                       [0, 0, 0, 1]], dtype=np.float32)
        translation_offset = Pose.from_4x4mat(translation_offset)
        body2sat = translation_offset @ body2sat
